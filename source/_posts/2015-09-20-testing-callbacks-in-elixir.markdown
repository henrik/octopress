...
In Elixir, an anonymous function can read outside variables but not change them. We could start a separate server process and make it hang on to this state, but that would be a bit of a bother.
