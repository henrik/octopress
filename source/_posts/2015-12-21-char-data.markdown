...
Erlang (and thus Elixir) has another quite similar concept called IO lists (or IO data). The difference is that those treat integers as raw bytes instead of as UTF-8 codepoints, so you might end up with binaries that are not UTF-8 strings:
